...
local increaseOddPixels=true

SpriteGrabber.grabSheet = function(sheetName, sheetExtension)        
	local lSuffix = suffix or ""   
	local sheetExtension = sheetExtension or ".png"
	local iSheet = {}
	local sData = require(sheetName..lSuffix).getSpriteSheetData()
	local fTags = {}
	local sTags = {}
	local fullTag = nil
	local shortTag = nil
	...
	local iSpriteSheet = sprite.newSpriteSheetFromData( sheetName..lSuffix..sheetExtension, sData )
	...
	function iSheet:grabSprite(requestedSprite,showBool,clips)
		...
		if #spriteFrames>0 then
			...
			--to overcome the bug. See example here: http://developer.anscamobile.com/forum/2010/10/29/spritegrabber-class-sp...) 
		...
		function returningSprite:show(x,y,refPoint)
			...
			self.xScale, self.yScale = spriteScale, spriteScale
			self.isVisible=true
		end  
